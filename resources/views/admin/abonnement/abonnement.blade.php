                ...
                <table class="table payment-table">
                    ...
                    <tbody>
                        @forelse($owners as $owner)
                            <tr class="text-center">
                                <td><strong>{{ $owner->name.' '.$owner->prenom }}</strong></td>
                                <td class="amount-cell">{{ number_format($owner->montant, 0, ',', ' ') }} FCFA</td>
                                <td>
                                    @if($owner->methode_owner === 'Espèces')
                                        <span class="payment-method-icon"><i class="fas fa-money-bill-wave"></i></span>
                                    @elseif($owner->methode_owner === 'Mobile Money')
                                ...
                                <td>
                                    @if($owner->verif_espece)
                                        <span class="badge bg-light text-dark">{{ $owner->verif_espece }}</span>
                                    @else
                                        <span class="text-muted">N/A</span>
                                    @endif
                                </td>
                                <td>{{ \Carbon\Carbon::parse($owner->mois_couvert)->translatedFormat('F Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($owner->date_owner)->translatedFormat('d/m/Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($owner->created_at)->format('H:i') }}</td>
                                <td>
                                    @if($owner->statut === 'payé')
