...
@section('content')
    ...
    <div class="col-12 grid-margin stretch-card mb-4">
        <div class="card">
            <div class="card-body">
                ...
                <div class="modal fade" id="etatLieuModal" tabindex="-1" aria-labelledby="etatLieuModalLabel"
                    aria-hidden="true">
                            ...
                            <form id="statusForm" method="POST">
                                ...
                                <div class="modal-footer">
                                    ...
                                    <button type="submit" class="btn text-white "
                                        style="background-color: #02245b;">Enregistrer</button>
                                </div>
                            </form>
                        </div>
                    </div>
                                ...
                                <tr class="text-center pt-3" style="height: 30px">
                                    ...
                                    <td class="text-center ">
                                        <div class="btn-group gap-10" role="group">
                                            <a href="{{ route('locataire.admin.edit', $locataire->id) }}"
                                            ...
                                            @endif
                                            @if ($locataire->status === 'Actif' && $locataire->bien_id)
                                                <a href="{{ route('demenagement.show', $locataire->id) }}"
                                                    class="btn btn-sm btn-danger" title="Déménagement">
                                                    <i class="mdi mdi-home-export-outline"></i>
                                                </a>
                                            @endif
                                            <a href="{{ route('demenagement.historique', $locataire->id) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.change-status-btn').on('click', function() {
                ...
                form.attr('action',
                    "{{ route('locataire.admin.updateStatus', ['locataire' => 'LOCATAIRE_ID']) }}"
        ...
        $(document).on('click', '.attribuer-bien-btn', function() {
            ...
            `,
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: "{{ route('locataire.attribuer-bien', ['locataire' => 'LOCATAIRE_ID']) }}"
                                .replace('LOCATAIRE_ID', locataireId),
                            method: 'POST',
                            data: {
    ...
    <script>
        ...
        $(document).on('click', '.assign-comptable-btn', function() {
            ...
            $.ajax({
                url: "{{ route('comptables.recouvrement.admin') }}",
                type: 'GET',
                ...
                `,
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.ajax({
                                url: "{{ route('locataire.assign.comptable.admin') }}",
                                type: 'POST',
                                data: result.value,
                                success: function(response) {
