...
class AbonnementController extends Controller
{
    ...
    // =============================================
    // WAVE ABONNEMENT - PROPRIÉTAIRE
    // =============================================

    public function waveInitiateOwner(Request $request, $abonnementId)
    {
        $request->validate([
        ...
        $result = $waveService->createCheckoutSession([
            'amount'           => (int) $request->montant,
            'currency'         => 'XOF',
            'success_url'      => str_replace('http://', 'https://', route('abonnement.wave.success.owner')),
            'error_url'        => str_replace('http://', 'https://', route('abonnement.wave.error.owner')),
            'client_reference' => 'ABN-OWNER-' . $abonnement->id,
        ]);
    ...
    public function waveSuccessOwner(Request $request)
    {
        $data = session('wave_abonnement_owner');
        if (!$data) {
            return redirect()->route('owner.abonnement.show')->with('error', 'Session expirée. Veuillez réessayer.');
        }

        $abonnement = Abonnement::findOrFail($data['id']);

        $dateFin = $abonnement->type === $data['type']
            ? Carbon::parse($abonnement->date_fin)->addMonths($data['duree'])
            : now()->addMonths($data['duree']);
        ...
        $abonnement->update([
            ...
            'mode_paiement'       => 'Wave',
            'statut'              => 'actif',
            'reduction_appliquee' => $data['reduction'],
        ]);

        session()->forget('wave_abonnement_owner');

        return redirect()->route('owner.abonnement.show')
            ->with('success', 'Abonnement renouvelé avec succès via Wave jusqu\'au ' . $dateFin->format('d/m/Y'));
    }

    public function waveErrorOwner(Request $request)
    {
        return redirect()->route('owner.abonnement.show')
            ->with('error', 'Le paiement Wave a échoué ou a été annulé.');
    }

    // =============================================
    // WAVE ABONNEMENT - AGENCE
    // =============================================

    public function waveInitiateAgence(Request $request, $abonnementId)
    {
        $request->validate([
            'type'      => 'required|in:standard,premium',
            'duree'     => 'required|integer|in:1,3,6,12',
        ...
        session(['wave_abonnement_agence' => [
            'id'        => $abonnement->id,
            'type'      => $request->type,
            'duree'     => (int) $request->duree,
            'montant'   => (float) $request->montant,
        ...
        $result = $waveService->createCheckoutSession([
            'amount'           => (int) $request->montant,
            'currency'         => 'XOF',
            'success_url'      => str_replace('http://', 'https://', route('abonnement.wave.success.agence')),
            'error_url'        => str_replace('http://', 'https://', route('abonnement.wave.error.agence')),
            'client_reference' => 'ABN-AGENCE-' . $abonnement->id,
        ]);
    ...
    public function waveSuccessAgence(Request $request)
    {
        $data = session('wave_abonnement_agence');
        if (!$data) {
            return redirect()->route('agence.abonnement.show')->with('error', 'Session expirée. Veuillez réessayer.');
        }
        ...
        session()->forget('wave_abonnement_agence');

        return redirect()->route('agence.abonnement.show')
            ->with('success', 'Abonnement renouvelé avec succès via Wave jusqu\'au ' . $dateFin->format('d/m/Y'));
    }

    public function waveErrorAgence(Request $request)
    {
        return redirect()->route('agence.abonnement.show')
            ->with('error', 'Le paiement Wave a échoué ou a été annulé.');
    }
