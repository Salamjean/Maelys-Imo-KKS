...
class EtatAgentLocataire extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Lier à l'historique de location en cours
        $historique = HistoriqueLocation::where('locataire_id', $validated['locataire_id'])
            ->where('bien_id', $validated['bien_id'])
            ->whereNull('date_sortie')
            ->latest()
            ->first();
        if ($historique && !$historique->etat_lieu_entree_id) {
            $historique->update(['etat_lieu_entree_id' => $etatLieu->id]);
        }
